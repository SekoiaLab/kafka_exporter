...
func (e *Exporter) collect(ch chan<- prometheus.Metric) {
	...
	getTopicMetrics := func(topic string) {
		...
		e.mu.Unlock()
		// ponytail: bounded in-flight offset fetches per topic.
		// Upstream ran these serially: one 3.8k-partition topic
		// cost 7.7k sequential round trips of wall clock.
		pw := e.partitionWorkers
		if pw < 1 {
			pw = 1
		}
		var pwg sync.WaitGroup
		psem := make(chan struct{}, pw)
...
func main() {
	...
	toFlagIntVar("partition.workers",
		"In-flight offset fetches per topic (1 = upstream serial)",
		1, "1", &opts.partitionWorkers)
